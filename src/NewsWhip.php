...
class NewsWhip {
	...
	public function getClient() {
		...
		if (!self::$_client instanceof GuzzleClient) {
			self::$_client = new GuzzleClient(['base_uri' => $this->_baseUri]);
		}
	...
	public function setClient($client) {
		...
		if ($client instanceof GuzzleClient) {
			...
			if (!array_key_exists('base_uri', $client)) {
				$client['base_uri'] = $this->_baseUri;
			}
	 ...
	 */
	private function _fetch($uri, $method = 'post', $options = [], $postFields = []) {

		$client = $this->getClient();

		if (!array_key_exists(RequestOptions::QUERY, $options) || !is_array($options[RequestOptions::QUERY])) {
		...
		if (!array_key_exists('key', $options[RequestOptions::QUERY])) {
			$options[RequestOptions::QUERY]['key'] = $this->getApiKey();
		}

		if (!array_key_exists('Content-Type', $options[RequestOptions::HEADERS])) {
	...
	public function getByRegion($region, $category, $timePeriod, $size = 100, $sortBy = 'default', $videoOnly = false) {
		...
		return $this->_fetch(
			...
			'get',
			[RequestOptions::QUERY => ['size' => $size, 'sort_by' => $sortBy, 'video_only' => $videoOnly]]
		);
	...
	public function getBySearch($search, $size = 100, $sortBy = 'default', $videoOnly = false) {
		...
		return $this->_fetch(
			'search',
			'get',
			[RequestOptions::QUERY => ['size' => $size, 'sort_by' => $sortBy, 'video_only' => $videoOnly, 'q' => $search]]
		);
	...
	public function getArticles($filters = []) {
		...
		return $this->_fetch('publisher', 'post', [], $filters);
	 ...
	 * @param string       $type articles or stats
	 * @param bool         $autoFormat
	 *
	 * @return string
	 * @throws \Exception
	 */
	protected function _processFilters($filters, $type = 'articles', $autoFormat = true) {
